...
void testBendingConstraint()
{
    ...
    const auto constraint = std::make_shared<elasty::BendingConstraint>(
        p_0, p_1, p_2, p_3, 1.0, 0.0, dt, dihedral_angle);
...
void testIsometricBendingConstraint()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 3; ++j)
        {
            ...
            const double cost_plus =
                isometric_bending_constraint->calculateValue();

            particles[i]->p = particles[i]->x - eps3d;
            const double cost_minus =
                isometric_bending_constraint->calculateValue();
