...
std::vector<float> elasty::packParticlePositions(
...
void elasty::setRandomVelocities(
    const std::vector<std::shared_ptr<Particle>>& particles, const double scale)
{
    for (const auto& particle : particles)
    {
...
void elasty::generateFixedPointConstraints(
    const Eigen::Vector3d&                        search_position,
...
public:
          ...
          m_archive(Alembic::AbcCoreOgawa::WriteArchive(), file_path.c_str())
    {
        ...
        const uint32_t     time_sampling_index =
    ...
    void submitCurrentStatus()
    {
        ...
        const std::vector<float> verts =
            packParticlePositions(m_cloth_sim_object->m_particles);
        ...
        if (m_is_first)
        {
            const size_t num_indices =
                m_cloth_sim_object->m_triangle_list.size();
            const std::vector<int32_t> indices = [&]() {
                std::vector<int32_t> indices;
                indices.reserve(num_indices);
                for (unsigned int i = 0;
                     i < m_cloth_sim_object->m_triangle_list.rows();
                     ++i)
                {
                    indices.push_back(
                        m_cloth_sim_object->m_triangle_list(i, 0));
                    indices.push_back(
                        m_cloth_sim_object->m_triangle_list(i, 1));
                    indices.push_back(
                        m_cloth_sim_object->m_triangle_list(i, 2));
                }
                return indices;
            }();
            const size_t num_counts =
                m_cloth_sim_object->m_triangle_list.rows();
            ...
            const OV2fGeomParam::Sample geom_param_sample =
                m_cloth_sim_object->hasUv()
                    ? OV2fGeomParam::Sample(
                          V2fArraySample(
                              (const V2f*) m_cloth_sim_object->m_uv_list.data(),
                              num_indices),
                          kFacevaryingScope)
                    : OV2fGeomParam::Sample();

            const OPolyMeshSchema::Sample sample(
                V3fArraySample((const V3f*) verts.data(), num_verts),
...
std::shared_ptr<elasty::AlembicManager> elasty::createAlembicManager(
...
void elasty::submitCurrentStatus(
    const std::shared_ptr<AlembicManager> alembic_manager)
{
